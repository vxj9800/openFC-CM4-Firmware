...
{
    // We are asserting that the length is <= maxPacketSize. This check is not helpful
    // for bulk transfers, which will not be done here.
    assert(len <= ep->descriptor->wMaxPacketSize);
...
void isr_usbctrl(void)
{
    ...
    if (status & USB_INTS_SETUP_REQ_BITS)
    {
        ...
        struct usb_setup_packet pkt = *((volatile struct usb_setup_packet *)&usb_dpram->setup_packet);
        dev_config.setupPktHandler(&pkt);
    }
